...
static const char *TTAG = "DS18B20";
...
void find_onewire(ds18b20_device_handle_t *ds18b20s,
                  uint8_t *ds18b20_device_num) {
  ...
  do {
    ...
    if (search_result == ESP_OK) { // found a new device, let's check if we can
      ...
      if (ds18b20_new_device(&next_onewire_device, &ds_cfg,
        ...
        ESP_LOGI(TTAG, "Found a DS18B20[%d], address: %016llX",
                 *ds18b20_device_num, next_onewire_device.address);
        *ds18b20_device_num += 1;
      } else {
        ESP_LOGI(TTAG, "Found an unknown device, address: %016llX",
                 next_onewire_device.address);
      }
    }
  } while (search_result != ESP_ERR_NOT_FOUND);
  ESP_ERROR_CHECK(onewire_del_device_iter(iter));
  ESP_LOGI(TTAG, "Searching done, %d DS18B20 device(s) found",
           *ds18b20_device_num);
}
